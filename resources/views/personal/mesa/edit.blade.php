...
@section ('contenido') <!--abrimos la seccion donde estara nuestro contenido dinamico donde 'contenido' es el que definimos en la plantilla-->
    <div class="row ml-1 mr-1 p-2 border bg-light">
        <div class="col-lg-10 col-md-10 col-sm-10 col-xs-12">
            <h4>EDITAR MESA: <nom>{{ $mesa->num_mesa }}</nom></h4>
            @if (count($errors)>0)
    ...
    {!!Form::model($mesa,['method'=>'PATCH','route'=>['mesa.update',$mesa->idmesa]])!!}
    {{Form::token()}}
    <div class="row ml-1 mr-1 p-2 border">
        <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
            <div class="form-group">
                <label for="num_mesa">Numero de Mesa</label>
                <input type="text" name="num_mesa" required value="{{ $mesa->num_mesa }}" class="form-control" placeholder="Mesa...">
            </div>
        </div>
        <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
            <div class="form-group">
                <label for="cant_per">Capacidad de Personas</label>
                <input type="text" name="cant_per" value="{{ $mesa->cant_per }}" class="form-control" placeholder="Cantidad de Personas...">
            </div>
        </div>
