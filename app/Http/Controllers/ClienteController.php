...
class ClienteController extends Controller
{
    ...
    public function index(Request $request)
    {
        if ($request)
        {
            ...
            ->orderBy('nombre','asc')
            ->paginate(10);
            return view('ventas.cliente.index',["clientes"=>$clientes,"searchText"=>$query,"n"=>$n]);
        }
    ...
    public function store(PersonaFormRequest $request)
    {
        ...
        $cliente = new Persona; //creamos un nuevo objeto (una categoria)
        $cliente->tipo_persona = 'cliente'; //al campo nombre le pasamos el nombre obtenido del formulario html
        $cliente->nombre = $request->get('nombre'); //al campo estado le asignamos 1 por default
        $cliente->tipo_documento = $request->get('tipo_documento'); //al campo estado le asignamos 1 por default
        $cliente->num_documento = $request->get('num_documento');
        $cliente->direccion = $request->get('direccion');
        $cliente->telefono = $request->get('telefono');
        $cliente->f_nacimiento = $request->get('f_nacimiento');
        $cliente->email = $request->get('email');
        $cliente->estado = '1';
        $cliente->save();                           //guardamos la categoria en la base de datos
        return Redirect::to('ventas/cliente');     //redireccionamos a almacen/categoria donde va estar nuestro index(pagina principal) para categoria.
    }
    ...
    public function show($id)
    {
        ...
        $visitas = DB::table('venta')
        ->select(DB::RAW('COUNT(idventa) as cant'))
        ->where('idcliente','=',$id)
        ->first();

        return view("ventas.cliente.show",["cliente"=>$cliente,"visitas"=>$visitas]);//llama a la vista show y le envia la categoria id
    }
    ...
    {
        $cliente = Persona::findOrFail($id); //traigo al objeto categoria mediante el id el cual se almacena en $categoria
        $cliente->nombre = $request->get('nombre'); //al campo estado le asignamos 1 por default
        $cliente->tipo_documento = $request->get('tipo_documento'); //al campo estado le asignamos 1 por default
        $cliente->num_documento = $request->get('num_documento');
        $cliente->direccion = $request->get('direccion');
        ...
        $cliente->f_nacimiento = $request->get('f_nacimiento');
        $cliente->email = $request->get('email');
        $cliente->update(); //actualiza el objeto categoria
        return Redirect::to('ventas/cliente');     //redireccionamos a almacen/categoria donde va estar nuestro index(pagina principal) para categoria
    }
