...
class CargoController extends Controller
{
    ...
    public function store(CargoFormRequest $request)
    {
        ...
        $cargo = new Cargo; //creamos un nuevo objeto (una categoria)
        $cargo->nombre = $request->get('nombre'); //al campo nombre le pasamos el nombre obtenido del formulario html
        $cargo->sueldo = $request->get('sueldo'); //al campo estado le asignamos 1 por default
        $cargo->save();                           //guardamos la categoria en la base de datos
        return Redirect::to('personal/cargo');     //redireccionamos a almacen/categoria donde va estar nuestro index(pagina principal) para categoria.
    }
    ...
    {
        return view("personal.cargo.show",["cargo"=>Cargo::findOrFail($id)]);//llama a la vista show y le envia la categoria id
    }
    ...
    {
        return view("personal.cargo.edit",["cargo"=>Cargo::findOrFail($id)]);
    }
    ...
    {
        $cargo = Cargo::findOrFail($id); //traigo al objeto categoria mediante el id el cual se almacena en $categoria
        $cargo->nombre = $request->get('nombre'); //aqui el nombre sera el que este dentro de nuestro objeto nombre del formulario html
        $cargo->sueldo = $request->get('sueldo'); //aqui el nombre sera el que este dentro de nuestro objeto nombre del formulario html
        $cargo->update(); //actualiza el objeto categoria
        return Redirect::to('personal/cargo');     //redireccionamos a almacen/categoria donde va estar nuestro index(pagina principal) para categoria
    }
